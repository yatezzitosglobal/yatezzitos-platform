<ul class="item-amenities <?php echo houzez_v1_4_meta_type(); ?> d-flex flex-wrap align-items-center gap-2 mb-2"">
				...
				if( $custom_field_value != '' ) { 
					$output .= '<li class="h-'.$key.'" d-flex align-items-center me-1>';
						...
						if(houzez_option('icons_type') == 'font-awesome') {
							$output .= '<i class="'.houzez_option('fa_'.$key).' me-1"></i>';

						} elseif (houzez_option('icons_type') == 'custom') {
							$cus_icon = houzez_option($key);
							if(!empty($cus_icon['url'])) {
								$output .= '<img class="img-fluid me-1" src="'.esc_url($cus_icon['url']).'" width="16" height="16" alt="'.esc_attr($cus_icon['title']).'">';
							}
						}

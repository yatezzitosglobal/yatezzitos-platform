...
if( $enable_status || $enable_label ) {
	if( !empty($term_status) && $enable_status ) {
		foreach( $term_status as $status ) {
	        ...
	        echo '<a href="'.get_term_link($status_id).'" class="label-status label me-1 status-color-'.intval($status_id).'">
	...
	if( !empty($term_label) && $enable_label ) {
	    foreach( $term_label as $label ) {
	        ...
	        echo '<a href="'.get_term_link($label_id).'" class="hz-label label me-1 label-color-'.intval($label_id).'">
					'.esc_attr($label_name).'
				</a>';
	    }
	}

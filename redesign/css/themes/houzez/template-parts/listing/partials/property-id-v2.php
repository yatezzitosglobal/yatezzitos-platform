...
if( $property_id != '' ) { 
	$output .= '<li class="h-property-id">';
		...
		if(houzez_option('icons_type') == 'font-awesome') {
			$output .= '<i class="'.houzez_option('fa_property-id').' ms-1"></i>';
			...
			if(!empty($cus_icon['url'])) {
				...
				$output .= '<img class="img-fluid ms-1" src="'.esc_url($cus_icon['url']).'" width="16" height="16" alt="'.esc_attr($alt).'">';
			}
		} else {
			$output .= '<i class="houzez-icon icon-tags ms-1"></i>';
		}

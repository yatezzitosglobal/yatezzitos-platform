...
if( !function_exists('houzez_listing_price') ) {
    function houzez_listing_price($attr = 'span') {
        ...
        if( houzez_is_price_placeholder($listing_id) ) {
            return houzez_is_price_placeholder($listing_id, $attr); 
        }
        ...
        if( $listing_price ) {
            ...
            if($short_prices != 1 ) {
                ...
                if ( class_exists( 'FCC_Rates' ) && houzez_currency_switcher_enabled() && isset( $_COOKIE['houzez_set_current_currency'] ) ) {
                    ...
                    if ( Fcc_currency_exists( $current_currency ) ) {
                        $base_currency = houzez_default_currency_for_switcher();
                        $listing_price = Fcc_convert_currency( $listing_price, $base_currency, $current_currency );
                        $currency_data = Fcc_get_currency( $current_currency );
                        $listings_currency = $currency_data['symbol'];
                        $listing_currency_pos = $currency_data['position'];
                    }
                }
...
if( ! function_exists('houzez_is_price_placeholder') ) {
    function houzez_is_price_placeholder( $listing_id, $attr = 'li' ) {
        ...
        if( $is_price_placeholder && ! empty( $price_placeholder ) ) {
            ...
            if( is_singular( 'property' ) ) {
                $output .= '<'.$attr.' class="item-price item-price-text price-single-listing-text">'.$price_placeholder. '</'.$attr.'>';
                return $output;
            }

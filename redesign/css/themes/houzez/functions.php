...
define( 'HOUZEZ_THEME_VERSION', '4.3.1' );
...
require_once( HOUZEZ_FRAMEWORK . 'functions/property_schema.php' );
require_once( HOUZEZ_FRAMEWORK . 'functions/captcha-functions.php' ); // Unified captcha system
...
// Load Favethemes License Manager (theme-integrated)
require_once( HOUZEZ_FRAMEWORK . 'flm-loader.php' );
...
if ( !function_exists( 'houzez_block_users' ) ) {
	...
	function houzez_block_users() {
		...
		if( is_user_logged_in() && $users_admin_access && !houzez_is_demo() ) {
			...
			if (is_admin() && !current_user_can('administrator') && !houzez_is_editor() && isset( $_GET['action'] ) != 'delete' && !(defined('DOING_AJAX') && DOING_AJAX)) {
...
/**
 * Preload featured image on property detail pages for better LCP
 */
if( ! function_exists('houzez_preload_property_featured_image') ) {
	function houzez_preload_property_featured_image() {
		...
		if ( ! $featured_image_id ) {
			// Try to get first gallery image if no featured image
		...
		// Get appropriate image size based on layout - use same dynamic size as gallery template
		...
		if ( $image_data && isset( $image_data[0] ) ) {
			...
			// Output simplified preload link tag for fastest LCP
			// No srcset to ensure browser loads exact image immediately
			echo '<link rel="preload" as="image" href="' . esc_url( $image_url ) . '" fetchpriority="high">' . "\n";
		}
	}
	add_action('wp_head', 'houzez_preload_property_featured_image', 1);
}

/**
 * One-time cleanup of legacy houzez_template_* options from wp_options.
 * These were previously used to cache individual template data but are no longer needed
 * since templates are now served from JSON files via the favethemes-api plugin.
 */
if ( ! function_exists( 'houzez_cleanup_legacy_template_options' ) ) {
	function houzez_cleanup_legacy_template_options() {
		if ( get_option( 'houzez_template_options_cleaned' ) ) {
			return;
		}

		global $wpdb;
		$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'houzez\_template\_%'" );

		delete_option( 'houzez_sync_progress' );
		delete_option( 'houzez_templates_last_sync' );

		update_option( 'houzez_template_options_cleaned', 1, false );
	}
	add_action( 'admin_init', 'houzez_cleanup_legacy_template_options' );
}
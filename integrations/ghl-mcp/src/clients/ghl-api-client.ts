...
export class GHLApiClient {
  ...
  async createEmailTemplate(params: MCPCreateEmailTemplateParams): Promise<GHLApiResponse<any>> {
    try {
      ...
      const payload: any = {
        locationId: this.config.locationId,
        type: 'html',
        editorType: 'html', // force raw html editor instead of builder 
        ...params
      };
      
      // GHL expects 'html' to be inside 'templateData'
      if (params.html) {
        payload.templateData = { html: params.html };
        // We can leave params.html at the root just in case, but templateData is what the builder reads
      }

      const response: AxiosResponse<any> = await this.axiosInstance.post('/emails/builder', payload);
      return this.wrapResponse(response.data);
    } catch (error) {
      throw this.handleApiError(error as AxiosError<GHLErrorResponse>);
    }
  ...
  async updateEmailTemplate(params: MCPUpdateEmailTemplateParams): Promise<GHLApiResponse<any>> {
    try {
      const { templateId, html, ...data } = params;
      const payload: any = {
        locationId: this.config.locationId,
        templateId,
        ...data,
        editorType: 'html', // force code editor instead of drag-and-drop
        updatedBy: (data as any).updatedBy || 'GoHighLevel-MCP'
      };
      
      if (html) {
        payload.html = html; // Required by /emails/builder/data at the root
        payload.templateData = { html };
      }

      const response: AxiosResponse<any> = await this.axiosInstance.post('/emails/builder/data', payload);
      return this.wrapResponse(response.data);
    } catch (error) {
      throw this.handleApiError(error as AxiosError<GHLErrorResponse>);
    }
